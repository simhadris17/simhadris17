...
editing projects.json â€” the README never changes.
...
# active palette â€” set by set_theme(); defaults to dark
...
def wrap_text(s, max_chars, max_lines=2):
    ...
    if len(lines) == max_lines and words and " ".join(lines).count(" ") + 1 < len(words):
        lines[-1] = lines[-1][:max_chars-1].rstrip() + "â€¦"
    return lines
...
def card(p, x, y, idx):
    ...
    # logo (base64) or fallback monogram â€” with a gentle vertical float
    ...
    # language donut, animated draw-in â€” vertically centered in the card body
